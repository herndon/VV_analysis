...
#include "Services/include/UniqueRootDirectory.h"
...
TFile * getTFileOrThrow( std::string const& name ) {
    ...
    if ( !gFile ) {
        throw vvana::Exception("UniqueRootDirectory::getTFileOrThrow: for directory named "
                            + name
...
std::string ensureUnique( std::string const& baseName) {
    ...
    throw vvana::Exception("UniqueRootDirectory::ensureUnqiue: for directory named "
                        + baseName
...
vvana::UniqueRootDirectory::UniqueRootDirectory ( std::string const& name ):
...
vvana::UniqueRootDirectory::~UniqueRootDirectory () {
    _savedDirectory->cd();
}

void vvana::UniqueRootDirectory::addToFile ( TNamed* obj,
